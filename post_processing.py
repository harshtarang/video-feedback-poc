    ...
    for fb in fdbck:
        ...
        while True:
            if prev_id >= 0 and prev_id < len(sentence_df):
                ...
                sentence = dfr["sentence"].iloc[0].translate(str.maketrans('', '', string.punctuation)).lower()
...
def process_llm_feedback(feedback, sen_df, feedback_type, max_num_feedback):
   ...
   if feedback_type == "audio" or feedback_type == "text":
       ...
       final_pos_feedback = select_feedback(pos_fdbck, sen_df, feedback_type, max_num_feedback)
       final_neg_feedback = select_feedback(neg_fdbck, sen_df, feedback_type, max_num_feedback) 
       return final_pos_feedback, final_neg_feedback
   if feedback_type == "quality":
       q_fdbck = content["quality_feedback"]
       final_q_feedback = select_feedback(q_fdbck, sen_df, feedback_type, max_num_feedback)
...
def collate_all_feedback(sentence_transcript, audio_feedback, text_feedback, quality_feedback):
    ...
    neg_q = process_llm_feedback(quality_feedback, sentence_df, "quality", MAX_NUM_AUDIO_FEEDBACK)
